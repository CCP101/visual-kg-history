    ...
    * */
    ...
    async function QuizSavetoSQL(Path) {
        ...
        for (let i = 0; i < data.length; i++) {
            let row = data[i];
            let quiz_save_sql = "INSERT INTO quiz_save(quiz_id,id,exam_uuid,quiz_question,quiz_c1,quiz_c2,quiz_c3,quiz_c4,quiz_A) " +
                `VALUES ('${row.quizid}','${row.id}','${row.uuid}','${row.quiz}','${row.quizC1}','${row.quizC2}','${row.quizC3}','${row.quizC4}','${row.quizC}')`;
            let response = await ConnectMysql(quiz_save_sql);
            if (response === undefined){
                console.log("NOT SUCCESS")
                return "500";
            }
