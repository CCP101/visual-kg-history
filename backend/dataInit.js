...
async function ImportDataToNeo4j() {
  ...
  for (const people of Nodeset) {
    ...
    const query = `CREATE (n:DPerson {name: '${people['name']}',id:'${people['name_id']}'})`;
  ...
  for (const row of result) {
    ...
    if (direction !== '') {
      if (direction === '1') {
        const query = `MATCH (p1:DPerson{name:'${p1}'}),(p2:DPerson{name:'${p2}'}) CREATE (p1)-[r:${rel}]->(p2)`;
        await NodesWrite(query, 'result');
      } else {
        const query = `MATCH (p1:DPerson{name:'${p1}'}),(p2:DPerson{name:'${p2}'}) CREATE (p1)<-[r:${rel}]-(p2)`;
        await NodesWrite(query, 'result');
      ...
      const query1 = `MATCH (p1:DPerson{name:'${p1}'}),(p2:DPerson{name:'${p2}'}) CREATE (p1)-[r:${rel}]->(p2)`;
...
async function calWeight() {
  ...
  for (const people of set) {
    const NeoQuery = `match ((n:DPerson{name:'${people}'})-[]-(x:DPerson)) return count(x)`;
    const NeoResult = await NodesRead(NeoQuery, 'count(x)');
    const query = `UPDATE people SET weight = ${NeoResult[0]} WHERE name = "${people}"`;
    await ConnectMysql(query);
  }
}
